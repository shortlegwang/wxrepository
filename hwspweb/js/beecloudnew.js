...
function doPay(payData, cbsuccess, cberror) {
	...
	w_new = plus.nativeUI.showWaiting();
	...
	mui.ajax(getRandomHost(), {
		...
		success: function(data) {
			...
			if (data.result_code == 0) {
				...
				console.log("pay start------------------"+payData.channel);
				var payChannel = getPayChannel(payData.channel);
				console.log("pay return------------------" + (payChannel));
				if (payChannel) {
					
					if (payChannel === 'alipay') {
						paySrc = data.order_string;
						console.log("pay return------------------" + paySrc);
